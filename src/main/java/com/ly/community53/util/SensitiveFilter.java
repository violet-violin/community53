...
public class SensitiveFilter {
    ...
    public void init() {
        try (
            ...
            String keyword;
            while ((keyword = reader.readLine()) != null) {//字符缓冲流一次读一行
                // 添加到前缀树
...
//            TrieNode trieNode = tempNode.subNodes.get(c);
            ...
            if (subNode == null) {//没有子节点，自己去初始化
                ...
                tempNode.addSubNode(c, subNode);//一个节点的子节点不限个数，随便挂
            }
            ...
            // 设置结束标识
            if (i == keyword.length() - 1) {//字符流的最后一个位置，打上标识
    ...
    public String filter(String text) {
        ...
        while (position < text.length()) {
            ...
            // 检查下级节点
            tempNode = tempNode.getSubNode(c);//根据c，查找有没有下级节点
            if (tempNode == null) {
                // 以begin开头的字符串不是敏感词；把begin指向字符加入sb
